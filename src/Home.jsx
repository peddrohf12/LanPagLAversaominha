...
const RisingEmbers = () => {
    const embers = useMemo(() => Array.from({ length: 40 }).map((_, i) => ({
        ...
        color: Math.random() > 0.5 ? 'bg-brand-pink' : 'bg-brand-purple'
    })), []);
    return (
        <div className="absolute inset-0 overflow-hidden pointer-events-none select-none">
            {embers.map((ember) => (
                <div key={ember.id} className={cn("absolute rounded-full blur-[1px] animate-ember-rise opacity-0", ember.color)} style={{ left: ember.left, width: ember.size, height: ember.size, '--duration': ember.duration, '--delay': ember.delay, bottom: '-5vh' }} />
            ))}
        </div>
    );
};
...
const MagicalBackground = () => (
  <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none bg-brand-dark">
    ...
    <motion.div animate={{ scale: [1, 1.1, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 12, repeat: Infinity }} className="absolute top-[-20%] left-[-10%] w-[80vw] h-[80vw] rounded-full bg-brand-purple/20 blur-[100px] mix-blend-screen" />
    <motion.div animate={{ scale: [1, 1.2, 1], x: [0, 30, 0] }} transition={{ duration: 15, repeat: Infinity }} className="absolute top-[10%] right-[-20%] w-[70vw] h-[70vw] rounded-full bg-brand-pink/15 blur-[100px] mix-blend-screen" />
    <RisingEmbers />
    <StarDust />
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center sm:p-4">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="absolute inset-0 bg-brand-dark/90 backdrop-blur-sm" />
          <motion.div 
...
const Header = ({ onAuth }) => (
  <header className="fixed top-0 w-full z-50 px-4 py-4 md:px-8 bg-brand-dark/60 backdrop-blur-xl border-b border-white/5 flex justify-between items-center transition-all duration-300">
...
const Hero = ({ onAuth }) => (
  <section className="relative min-h-[100dvh] flex flex-col justify-center items-center px-4 pt-24 pb-10 overflow-hidden text-center z-10">
    ...
    <FadeIn delay={0.2} className="mt-8 mb-6 relative z-20">
      <h1 className="font-serif text-4xl md:text-7xl lg:text-8xl font-medium text-white leading-[1.1] tracking-tight drop-shadow-lg">Eleve sua</h1>
      <div className="relative inline-block mt-2">
        <span className="font-serif text-5xl md:text-7xl lg:text-8xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-[#ff66c4] via-white to-[#a855f7] animate-shine bg-[length:200%_auto] drop-shadow-magical relative z-10">frequência vibracional</span>
    ...
    <FadeIn delay={0.4} className="max-w-xl mx-auto mt-6 relative z-20">
      <p className="text-gray-200 text-base md:text-xl leading-relaxed font-light drop-shadow">Descubra qual energia você está emitindo e por que atrai situações que não te valorizam. <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-pink to-brand-orange font-bold block mt-2 drop-shadow-sm">Mude sua vibração. O universo responde.</span></p>
    </FadeIn>
    <FadeIn delay={0.6} className="w-full max-w-sm mt-10 relative z-20 group">
      ...
      <div className="relative bg-brand-dark/80 backdrop-blur-xl border border-white/10 rounded-2xl p-6 overflow-hidden hover:border-brand-pink/30 transition-colors">
        <div className="absolute top-0 right-0 bg-gradient-to-r from-brand-pink to-brand-purple text-[10px] font-bold px-3 py-1 rounded-bl-lg text-white uppercase tracking-wider shadow-lg">Oferta Relâmpago</div>
        <div className="flex flex-col gap-1 mb-5">
          <span className="text-gray-400 text-sm line-through">De R$ 42,00</span>
          <div className="flex items-center justify-center gap-2"><span className="text-4xl font-bold text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">R$ 17,50</span><span className="bg-brand-pink/20 border border-brand-pink/30 text-brand-pink text-xs font-bold px-2 py-0.5 rounded animate-pulse">HOJE</span></div>
        </div>
        <AnimatedButton primary fullWidth onClick={() => onAuth('signup')} className="shadow-[0_0_30px_rgba(236,72,153,0.4)] py-4 text-lg">Quero Alinhar Minha Energia <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform"/></AnimatedButton>
        <p className="flex items-center justify-center gap-1.5 text-[11px] text-gray-400 mt-3"><ShieldCheck size={12} className="text-brand-pink"/> Acesso Imediato e Garantido</p>
      </div>
    </FadeIn>
  </section>
);
    ...
    <section className="py-24 px-4 md:px-8 relative z-10">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
            <FadeIn><h2 className="font-serif text-3xl md:text-5xl text-white mb-4 drop-shadow-lg">O que acontece quando você <br/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-pink to-brand-purple drop-shadow-magical italic">alinha sua energia?</span></h2></FadeIn>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {items.map((item, i) => (
            <FadeIn key={i} delay={i * 0.15}>
              <motion.div whileHover={{ y: -10, scale: 1.02 }} className="relative h-full bg-white/5 backdrop-blur-lg border border-white/10 p-8 rounded-3xl overflow-hidden group transition-all duration-500 hover:border-brand-pink/40 hover:shadow-card-hover">
                ...
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-purple to-brand-pink flex items-center justify-center text-white mb-6 shadow-lg shadow-brand-pink/30 group-hover:scale-110 group-hover:rotate-3 transition-all relative z-10"><item.icon size={28} /><Sparkles className="absolute -top-2 -right-2 text-white opacity-0 group-hover:opacity-100 animate-pulse" size={12}/></div>
...
const Testimonials = () => (
  <section className="py-24 px-4 relative z-10 bg-brand-dark/30">
    <div className="max-w-4xl mx-auto text-center">
      <FadeIn><MagicBadge text="Histórias Reais" icon={Star} /><h2 className="font-serif text-3xl md:text-4xl text-white mt-6 mb-12 drop-shadow-lg">Quem mudou de frequência, <br/> <span className="italic text-brand-pink">mudou de vida.</span></h2></FadeIn>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         {[
           ...
           <FadeIn key={i} delay={i * 0.2}>
             <motion.div whileHover={{ y: -5 }} className="bg-gradient-to-br from-brand-deepPurple/80 to-brand-dark border border-brand-purple/20 p-8 rounded-3xl text-left relative group hover:border-brand-pink/30 transition-colors">
               <div className="absolute top-4 left-6 text-brand-pink/20 font-serif text-8xl leading-none select-none">"</div>
               <div className="flex gap-1 mb-4 relative z-10">{[1,2,3,4,5].map(s => <Star key={s} size={14} className="fill-brand-orange text-brand-orange drop-shadow" />)}</div>
               <p className="text-gray-200 text-base italic mb-6 relative z-10 leading-relaxed">"{t.text}"</p>
               <div className="flex items-center gap-3 relative z-10">
                 <div className="w-10 h-10 rounded-full bg-gradient-to-r from-brand-purple to-brand-pink flex items-center justify-center text-white text-sm font-bold shadow-sm">{t.name.charAt(0)}</div>
                 <div><span className="text-white font-bold text-sm block">{t.name}</span><span className="text-brand-pink text-xs flex items-center gap-1"><CheckCircle size={10}/> Compra Verificada</span></div>
               </div>
             </motion.div>
           </FadeIn>
         ))}
    ...
    <div className="max-w-lg mx-auto relative">
       ...
       <div className="relative bg-brand-dark/90 backdrop-blur-2xl border border-brand-pink/30 rounded-[2.5rem] p-8 md:p-12 text-center overflow-hidden group hover:border-brand-pink/50 transition-colors">
         ...
         <div className="mb-8 relative z-10 inline-block">
             ...
             <div className="text-6xl font-bold text-white tracking-tight drop-shadow-[0_0_15px_rgba(236,72,153,0.5)]">R$17<span className="text-3xl text-gray-300">,50</span></div>
         ...
         <ul className="text-left space-y-4 mb-10 relative z-10 max-w-sm mx-auto bg-white/5 p-6 rounded-2xl border border-white/5">
            {["Diagnóstico Vibracional", "Mapa de Bloqueios", "Guia de Reprogramação", "Bônus: Áudios & IA"].map((feat, i) => (
              <li key={i} className="flex items-start gap-3 text-gray-200 text-sm md:text-base"><div className="mt-1 bg-brand-pink/20 p-1 rounded-full"><CheckCircle size={16} className="text-brand-pink flex-shrink-0" /></div><span>{feat}</span></li>
            ))}
         </ul>
         <AnimatedButton primary fullWidth onClick={() => onAuth('signup')} className="shadow-[0_0_40px_rgba(236,72,153,0.5)] animate-pulse-slow py-5 text-lg group">SIM, QUERO MINHA TRANSFORMAÇÃO <ArrowRight className="group-hover:translate-x-2 transition-transform" /></AnimatedButton>
         <div className="mt-8 flex items-center justify-center gap-2 text-xs text-gray-400 relative z-10 bg-brand-dark/50 p-3 rounded-xl border border-white/5"><ShieldCheck size={16} className="text-green-400 flex-shrink-0" /><span>Garantia incondicional de 7 dias.</span></div>
       </div>
       </FadeIn>
    </div>
  </section>
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-brand-dark flex items-center justify-center">
        <Sparkles className="text-brand-pink animate-spin" size={48} />
      </div>
    );
  }
